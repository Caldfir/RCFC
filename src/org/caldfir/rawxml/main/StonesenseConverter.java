...
public class StonesenseConverter {
	...
	public static void main(String[] args) {
		...
		FileProgressFrame display = new FileProgressFrame("df to ssense", 2 * fileList.length);
		...
		try {
			...
			try {
				for (int i = 0; i < fileList.length; i++) {
					...
					extension = inName.substring(inName.length() - 3,
							inName.length());
					// read and parse
					...
					if (extension.equals("txt")) {
						if (t.getRoot() != null) {
							...
							if (output != null) {
								display.set("writing " + shortName + ".xml",
										2 * i + 2);

								// write
								writer = new FileWriter(DST_FOLDER + "/"
										+ shortName + ".xml");
								...
								er.write("non-graphics file \"" + shortName
									+ ".txt\" was skipped");
						}
						else
							er.write("invalid or empty file: " + shortName
									+ ".txt");
					}
				}
	...
	/**
	 * Converts a loaded graphics file into a string representation 
	 *  appropriate for loading the graphics in Stonesense. 
	 * Returns null if this is not a graphics raw file.  
	 */
			...
			if (current.tagName().compareTo("CREATURE_GRAPHICS") == 0) {
				...
				for (int j=0; j<current.getChildCount(); j++) { 
					...
					if (filenames.containsKey(currentchild.getArgument(1))){
						...
						ErrorWriter er = ErrorWriter.getInstance();
						er.write("argument 1 was expected to be a tile page specifier: " + currentchild.toRawString());
						xml += "/>\n";
						continue;
					}
					...
					if(currentchild.tagLength() >= 5 
						...
						ErrorWriter er = ErrorWriter.getInstance();
						er.write("arguments 3 and 4 were expected to be integers: " + currentchild.toRawString());
					}
	...
	/**
	 * Parses the file, returns a map between the tile page descriptors and the relevant information.
	 */
	private static HashMap<String, GraphicsSheet> getFileMap(Tag root) {
		
		Tag current=null, currentchild=null;
		HashMap<String, GraphicsSheet> filenames = new HashMap<String, GraphicsSheet>();
		...
		for (int i=0; i<root.getChildCount(); i++) { 
			...
			if (current.tagName().compareTo("TILE_PAGE") == 0) {
				...
				String filename = null; int zoom = 0; int height = 0;
				...
				for (int j=0; j<current.getChildCount(); j++) { 
					...
					if (currentchild.tagName().compareTo("FILE") == 0) {
						...
						//check to see if these are equal powers of two - otherwise we just default to 0
				...
				if(filename != null){
					filenames.put(current.getArgument(1), new GraphicsSheet(filename,zoom,height));
				}
			}
		}
		return filenames;
